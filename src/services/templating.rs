...
mod tests {
    ...
    #[test]
    fn builds_context_with_secrets() {
        let mut values = HashMap::new();
        values.insert("name".into(), Value::String("Kakise".into()));
        let mut secrets = HashMap::new();
        secrets.insert("token".into(), Value::String("abc".into()));
        let context = build_context(&values, &secrets);
        assert_eq!(context["name"], "Kakise");
        assert_eq!(context["secrets"]["token"], "abc");
    }

    #[test]
    fn renders_templates() {
        let repo = tempfile::tempdir().unwrap();
        let template_path = repo.path().join("templates/hello.txt");
        ...
        let manifest = Manifest {
            ...
            templates: vec![TemplateMapping {
                source: PathBuf::from("templates/hello.txt"),
                destination: PathBuf::from(".hello"),
                mode: None,
            }],
        };
        let mut values = HashMap::new();
        values.insert("name".into(), Value::String("world".into()));
        let context = build_context(&values, &HashMap::new());
        let rendered = render_templates(repo.path(), &manifest, &context).unwrap();
